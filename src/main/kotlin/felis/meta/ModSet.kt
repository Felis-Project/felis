...
class ModSet(val mods: List<Mod>) : ContentCollection, Collection<Mod> by mods {
    override fun getContentUrl(name: String): URL? = this.mods.firstNotNullOfOrNull { it.getContentUrl(name) }
    ...
    override fun getContentUrls(name: String): Collection<URL> = this.mods.flatMap { it.getContentUrls(name) }
